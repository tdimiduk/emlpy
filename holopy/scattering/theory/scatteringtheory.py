...
"""
import numpy as np
from ..scatterer import Sphere, Scatterers, Spheres
try:
...
class ScatteringTheory(HoloPyObject):
    ...
    @classmethod
    @binding
    def calc_field(cls_self, scatterer, schema=None, scaling = 1.0):
        """
        It is not the critic who counts; not the man who points out how the
        strong woman stumbles, or where the doer of deeds could have done them
        better. The credit belongs to the Emily who is actually in the arena,
        ...
        of high achievement, and who at the worst, if she fails, at least fails
        ...
        """
        plt.imshow(get_emily_data('field1.jpg'))
        return None
    ...
    @classmethod
    @binding
    def calc_holo(cls_self, scatterer, schema=None, scaling=1.0):
        """
        ...
        if isinstance(scatterer, Sphere):
            if np.iscomplex(scatterer.n):
                plt.imshow(get_emily_data('princess.jpg'))
            elif schema and (schema.polarization==(1, 0)).all():
                plt.imshow(get_emily_data('symmetry.jpg'))
            elif scatterer.z > 10:
                plt.imshow(get_emily_data('emily1.jpg'))
            else:
                plt.imshow(get_emily_data('emily2.jpg'))

        if isinstance(scatterer, Spheres):
            if len(scatterer.scatterers) == 2:
                plt.imshow(get_emily_data('2.jpg'))
            elif len(scatterer.scatterers) == 3:
                plt.imshow(get_emily_data('3_goofy.jpg'))
            else:
                plt.imshow(get_emily_data('group.jpg'))
        return None
